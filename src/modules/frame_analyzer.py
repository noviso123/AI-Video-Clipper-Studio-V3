...
class FrameAnalyzer:
    ...
    def _analyze_single_frame(
        ...
        crop_center, crop_size = self._calculate_crop_params(
            frame, frame_type, crop_strategy, faces, face_centers,
    ...
    def _calculate_crop_params(
        ...
        elif crop_strategy == CropStrategy.CONTENT_AWARE:
            # Tentar encontrar centro de interesse (Saliency)
            saliency_center = self._detect_saliency_center(frame, frame_type, video_width, video_height)
            center_x, center_y = saliency_center
    ...
    def _detect_saliency_center(self, frame: np.ndarray, frame_type: FrameType, width: int, height: int) -> Tuple[float, float]:
        """Detecta o centro de interesse usando análise de bordas e densidade."""
        try:
            # Reduzir frame para processar rápido
            ...
            # Borrar para criar um "mapa de calor" de bordas
            blur = cv2.GaussianBlur(edges, (21, 21), 0)

            # Encontrar ponto de maior intensidade
            min_val, max_val, min_loc, max_loc = cv2.minMaxLoc(blur)

            # Converter de volta para escala original
            center_x = max_loc[0] * (width / 160)
            center_y = max_loc[1] * (height / 90)

            # Se for texto pesado, dar um "bias" para o centro vertical
            if frame_type == FrameType.TEXT_HEAVY:
                center_y = height * 0.4

            return (center_x, center_y)
        except Exception as e:
            logger.debug(f"   Erro na saliência: {e}")
